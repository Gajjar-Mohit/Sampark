...
export const creditBankAccount = async (
  ...
  try {
    ...
    console.log(`Attempting to credit ${amount} from account ${accountNo}`);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      console.log(
        `Current balance: ${account.balance}, Credit amount: ${amount}`
      );
    ...
    console.log(
      `Successfully credited ${amount}. New balance: ${result.updatedAccount.balance}`
    );
